...
class Transaction {
  ...
  static validateStandrdTransaction({ state, transaction }) {
    return new Promise((resolve, reject) => {
      ...
      if (value + gasLimit > fromBalance) {
        return reject(
          new Error(
            `Transaction value and gasLimit: ${value +
              gasLimit} exceeds balance: ${fromBalance}`
          )
        );
      }
  ...
  static validateCreateAccountTransaction({ transaction }) {
    return new Promise((resolve, reject) => {
      ...
      if (fields.length !== expectedAccountDataFields.length) {
        return reject(
          new Error(
            `The trnsaction account data has na incorrect number of fields`
          )
        );
      }
      ...
      fields.forEach(field => {
        if (!expectedAccountDataFields.includes(field)) {
          return reject(`The field: ${field}, is unexpected for account data`);
        }
      });
        ...
        try {
          switch (transaction.data.type) {
            ...
            case TRANSACTION_TYPE_MAP.TRANSACT:
              await Transaction.validateStandrdTransaction({
                state,
                transaction
              });
              break;
  ...
  static runTransaction({ state, transaction }) {
    switch (transaction.data.type) {
      case TRANSACTION_TYPE_MAP.TRANSACT:
        ...
        console.log(
          ' -- Update account data to reflect the standard transactions -- '
        );
        break;
      case TRANSACTION_TYPE_MAP.CREATE_ACCOUNT:
      ...
      case TRANSACTION_TYPE_MAP.MINING_REWARD:
        ...
        console.log(' -- Update account data to reflect the mining reward -- ');
        break;
      default:
        break;
    }
